 ...
 * a Tag is found. The Tag detection is made through the detectTag function of the
 ...
 *
 *******************************************************************************/
static bool moveRobot(NavigationModel * apModel, Protocol * apProtocol, int maxTagID)
{
   int counterToBreakdownPath = 0;
   ...
   while( !apModel->destinationIsReached() && possiblePathFlag)
   {
	 ...
	 // Position was reached without problem, send position vector right away
         if( cmdRcvd == REACHED )
         {
            ...
            if( BREAKDOWN_PATH_FLAG )
	    {
	       counterToBreakdownPath++;

	       // Checking for a tag instead of sending next position vector
	       if( counterToBreakdownPath == NUM_CELL_BETWEEN_TAG_DETECT )
	       {
...
//SWEEP TO DETECT TAG
                 if( tagDetection(apProtocol, maxTagID, tagID, xTag, zTag, angleCamMarker, camServoAngle) )
                  ...
                  // Attempt to detect a tag in front of the robot (Arduino sets camangle to 0 when it receives a vector)
//                  computervision compVision(CAM_PARAM);
//        	  if(  compVision.detectTag(maxTagID, tagID, xTag, zTag, angleCamMarker) )
                  {
                     camServoAngle = 0;
...
//                     fflush(file);
		     ...
		     // If tag is found, attempt to localize robot and calculate path
		     if( apModel->localizeRobotInGrid(tagID, xTag, zTag, angleCamMarker, camServoAngle) )
                     {
		        apModel->clearPath();
                        if( apModel->calculatePathToDest() )
                        {
			   cout << "Applied a correction -> a new path was found"<<endl;
//                           apModel->print(file);
                           apModel->print();
		        }
		        else
		        {
                           cout << "Unable to calculate path with correction. Stopping everything"<<endl;
//                           fprintf(file,"Unable to calculate path with correction. Stopping everything\n");
//                          fflush(file);
		     ...
		     {
                        cout << "Unable to localize robot in the grid to apply a correction"<<endl;
//                        fprintf(file,"Unable to localize robot in the grid to apply a correction.\n");
//                        fflush(file);
                     }
	          }
	       }
...
{
   bool firstDestFlag = true;
   errorType error;

   file = logfile();
   fprintf(file,"LOG FILE INITIATION\n");
   fflush(file);
...
{}
   ...
   else
   {
      ...
      while( true )
      {
         ...
         if( firstDestFlag && argc == 2 )
         {
            firstDestFlag = false;
            if( sscanf(argv[1], "%d", &goal_ix) )
            {
	       ...
	       {
 		   printf("Good job Deborah! You have set the destination to %d. The program will now crash...\n",goal_ix);

               }
            }
            else
            {
               printf("Error with argument. Stopping execution.\n");
               break;
            }
         }
         else
         ...
         printf("Destination ID is set to %d\n",goal_ix);
	 fprintf(file, "Destination ID is set to %d\n", goal_ix);
         fflush(file);
   Protocol protocol;
            ...
            if( model.localizeRobotInGrid(tagID, xTag, zTag, angleCamMarker, camServoAngle) )
	    {
               ...
               if( model.calculatePathToDest() )
	       {
	          ...
	          model.print();
                  // 4. MOVE TOWARD GOAL
	          if( moveRobot(&model, &protocol, maxTagID) ) // Return false if obstacles blocked all the possible path to destination
		  {
                     // Reset the grid before attempting to locate a tag for a correction	Ã
		     myGrid.resetGrid();
...
if(true)//	                if( tagDetection(&protocol, maxTagID, tagID, xTag, zTag, angleCamMarker, camServoAngle) )
    	                {
                           cout << "Tag " << tagID << ", xTag: "<< xTag << ", zTag: "<< zTag << ", angle " <<angleCamMarker << ",camangle " << camServoAngle<<endl;
                           fprintf(file, "Tag %d, xTag: %.02f, zTag; %.02f, angle: %.02f, camangle: %.02f\n",tagID,xTag,zTag,angleCamMarker,camServoAngle);
                           fflush(file);
                           if( !model.localizeRobotInGrid(tagID, xTag, zTag, angleCamMarker, camServoAngle) )
			   {
			      cout << "Unable to localize robot in the grid to apply a correction. Robot should be close to destination"<<endl;
                              fprintf(file,"Unable to localize robot in the grid to apply a correction. Robot should be close to destination\n");
                              fflush(file);
			   }
                        }
	                else
   		  	   ...
   		  	   if( model.calculatePathToDest() )
		           {
                              model.print(file);
	                      model.print();
                              moveRobot(&model, &protocol,maxTagID);
                     	   }
			   else
			   {
	   ...
	   {
		cout <<"Unable to locate robot inside the grid. Try to move it closer to a marker"<<endl;
	 	fprintf(file, "Unable to locate robot inside the grid. Try to move it closer to a marker\n");
		fflush(file);
	   }
	 }
